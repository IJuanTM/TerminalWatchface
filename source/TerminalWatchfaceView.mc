...
class TerminalWatchfaceView extends WatchUi.WatchFace {
    ...
    private function _drawLineRow(
        ...
        if (field == FIELD_STEPS) {
            var stepsView = _getProp("stepsViewMode", 0);
            if (stepsView == 1) {
                _drawStepsBarRow(dc, cx, y, labelColor, valueColor, false);
            } else if (stepsView == 2) {
                _drawStepsBarRow(dc, cx, y, labelColor, valueColor, true);
            } else {
    ...
    private function _drawStepsBarRow(
        ...
        if (fillW > 0) {
            var fillColor =
                steps >= goal ? _colorFromIdx(1) : _colorFromIdx(valueColor);
    ...
    private function _drawSingleGraphLabels(
        ...
        dc.setColor(_gradColor(colorIdx, maxFrac), Graphics.COLOR_TRANSPARENT);
        dc.drawText(
            gx - 4,
            y - 4,
            _fontTiny,
        ...
        dc.setColor(_gradColor(colorIdx, minFrac), Graphics.COLOR_TRANSPARENT);
        dc.drawText(
            gx - 4,
            y + gh - _tinyFh + 4,
            _fontTiny,
