...
class TerminalWatchfaceView extends WatchUi.WatchFace {
    ...
    private function _resolveOneLine(
        ...
        if (phase >= 1 && phase <= 5) {
            ...
            if (f != FIELD_NONE) {
                _resolvedFields[idx] = f;
                _resolvedLabelC[idx] = _getProp(s[1], 8);
                _resolvedValueC[idx] = _getProp(s[2], 0);
                return;
            }
        }
        _resolvedFields[idx] = _getProp(key + "Primary", FIELD_NONE);
        _resolvedLabelC[idx] = _getProp(key + "PrimaryLabelColor", 8);
        _resolvedValueC[idx] = _getProp(key + "PrimaryValueColor", 0);
    }
    ...
    private function _drawDualGraphRow(
        ...
        if (viewMode == VIEW_GRAPH_VALUE) {
            ...
            if (field == FIELD_TEMP_WRIST) {
                dc.drawText(
                    _drawSmallTempNum(dc, vx, startY, cur1),
                    startY,
                    _fontSmall,
                    _wxUnit,
                    Graphics.TEXT_JUSTIFY_LEFT
            ...
            if (fieldSecondary == FIELD_TEMP_WRIST) {
                dc.drawText(
                    _drawSmallTempNum(dc, vx, y2, cur2),
                    y2,
                    _fontSmall,
                    _wxUnit,
                    Graphics.TEXT_JUSTIFY_LEFT
    ...
    private function _drawForecastRow(
        ...
        _getGradRange(FIELD_WX_FORECAST_TEMP, lineColor, minV, range);
        var gradMinV = _grMin;
    ...
    private function _drawForecastWindRow(
        ...
        _getGradRange(FIELD_WX_FORECAST_WIND, lineColor, minV, range);
        var gradMinV = _grMin;
        var gradRange = _grRange;
        var maxFrac = (maxV - gradMinV) / gradRange;
        if (maxFrac < 0.0) {
    ...
    private function _drawDailyForecastRow(
        ...
        _getGradRange(FIELD_WX_FORECAST_TEMP, colorIdx, allMin, range);
        var gradMin = _grMin;
        var gradRange = _grRange;
        _rowBuf[0] = "Day Fcst";
        _rowBuf[1] = "";
        ...
        for (var si = 1; si < n; si++) {
            _drawDashedV(dc, gx + (si * gw) / n - 1, y, y + gh);
        }
        for (var i = 0; i < n; i++) {
        ...
        for (var di = 0; di < n; di++) {
            ...
            var slotX = gx + (di * gw) / n;
            var slotEnd = gx + ((di + 1) * gw) / n;
        ...
        if (viewMode == VIEW_GRAPH_VALUE) {
            ...
            if (valueMode == 2) {
                var minStr = _metric
                    ? allMin.format("%.0f")
                    : _toF(allMin).format("%.0f");
                var maxStr = _metric
                    ? allMax.format("%.0f")
                    : _toF(allMax).format("%.0f");
                dc.drawText(
                    vx,
                    vy,
                    _fontSmall,
    ...
    private function _drawGraphRow(
        ...
        if (viewMode == VIEW_GRAPH_VALUE) {
            ...
            if (field == FIELD_TEMP_WRIST) {
                dc.drawText(
                    _drawSmallTempNum(dc, valX, valY, valStr2),
                    valY,
                    _fontSmall,
                    _wxUnit,
                    Graphics.TEXT_JUSTIFY_LEFT
    ...
    private function _getFieldParts(field as Number) as Void {
        ...
        if (field == FIELD_HR_SPO2) {
            ...
            if (_acInfo != null) {
                var a = _acInfo as Activity.Info;
                if (a.currentHeartRate != null) {
                    hr = (a.currentHeartRate as Number).toString();
                }
                if (a.currentOxygenSaturation != null) {
                    spo2 =
    ...
    private function _formatPace(speedMps as Float) as String {
        ...
        var secPerDist = _metric ? 1000.0 / speedMps : 1609.344 / speedMps;
        var totalSec = secPerDist.toNumber();
        return (
            (totalSec / 60).format("%d") +
            ":" +
    ...
    private function _drawForecastHumidityRow(
        dc as Dc,
        cx as Number,
        y as Number,
        hours as Number,
        ...
        _getGradRange(FIELD_WX_FORECAST_HUMIDITY, lineColor, minV, range);
        var gradMinV = _grMin;
        var gradRange = _grRange;
        var maxFrac = (maxV - gradMinV) / gradRange;
        if (maxFrac < 0.0) {
    ...
    private function _drawForecastUvRow(
        ...
        _getGradRange(FIELD_WX_FORECAST_UV, lineColor, minV, range);
        var gradMinV = _grMin;
        var gradRange = _grRange;
        var maxFrac = (maxV - gradMinV) / gradRange;
        if (maxFrac < 0.0) {
    ...
    private function _drawForecastCloudRow(
        ...
        _getGradRange(FIELD_WX_FORECAST_CLOUD, lineColor, minV, range);
        var gradMinV = _grMin;
        var gradRange = _grRange;
        var maxFrac = (maxV - gradMinV) / gradRange;
        if (maxFrac < 0.0) {
